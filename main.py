...
# When called, the Questions class converts an entire file (".SHIVAM") to a Python Dictionary.
...
# Homescreen #
    ...
    def home(self):  # Homescreen Function 
        ...
        self.title = Label(self.homescreen, text="Shivam", bg="#46178f", fg="white",font=('Helvetica Neue',24,"bold"),wraplength=700,pady=5)
        ...
        for file in glob.glob("*.SHIVAM"): # Find .SHIVAM quiz files in current directory
        ...
        #       - Open  - Opens a filedialog where a .SHIVAM file can be selected if not already found by the browseQuiz Listbox
    ...
    # Select Quiz
    # The listbox ONLY has Quizes that have been found with the .SHIVAM file extension. Because of this, we can be confident that the Quiz file exists and is in the valid format
    ...
    def selectQuiz(self,other):
        ...
        try:
            self.filename=str(self.browseQuiz.get(self.browseQuiz.curselection())+".SHIVAM") # Find the file with the extension .SHIVAM
        ...
        self.meta= (json.load(open(str(self.browseQuiz.get(self.browseQuiz.curselection())+".SHIVAM")))) # Load file
    ...
    # Opens up a file dialog to find .SHIVAM files that arn't already found in this python files immediate directory
    def fileExplore(self):
        ...
        # The filedialog box can only open files with the extension .SHIVAM. It is unlikely that there are other files with the same extension so we can safely assume it is valid
        self.filename = filedialog.askopenfilename(initialdir = str(os.getcwd())+"/tests",title = "Select Shivam Quiz file",filetypes = [("Shivam Quiz Files","*.SHIVAM"),("Legacy Shivam Quiz Files","*.json")]) # Calls the file dialog box
...
# Quiz Window #
    ...
    def startQuiz(self):
        ...
        pygame.mixer.init()
        pygame.mixer.music.load("mp\start.mp3")
    ...
    def displayFrame(self):
        if self.itemsPacked == False: # If items have not been rendered
            # Shivam Title gets rendered before the frame so it appears at the top of the window
            self.title = Label(self.master, text="Shivam", bg="#46178f", fg="white",font=('Helvetica Neue',24,"bold"),wraplength=700,pady=5)
    ...
    def displayQuestion(self,other=None,Qnum=None): # If a number is parsed, then that number will be called to display
        ...
        pygame.mixer.init()
        pygame.mixer.music.load("mp\me.mp3")
    ...
    def answercheck(self,choice):
        ...
        if answer == choice: # If the user selected answer is the same as found in the dictionary
            ...
            pygame.mixer.init()
            pygame.mixer.music.load("mp\susp.mp3")
            pygame.mixer.music.play()
            # Append this question number to __answersCorrect variable
            print(str(getquestionnum)+": Correct") # Print question number and result
            pygame.mixer.init()
            pygame.mixer.music.load('rig.mp3')
            pygame.mixer.music.play()
...
# Shivam QuizCreator #
    ...
    def createQuiz(self):
        ...
        # create header title
        self.title = Label(self.mainarea, text="Shivam Quiz Creator", bg="#46178f", fg="white",font=('Helvetica Neue',24,"bold"),wraplength=700,pady=5)
    ...
    # Exports dictionary to .SHIVAM file
    ...
    def export(self):
        ...
        # Create an export button which when pressed, will export the file to (Quiz name).SHIVAM
    ...
    # Exports the file to (Quiz name).SHIVAM
    def exportFile(self):
        ...
        if glob.glob(self.quizNameVar.get()+".SHIVAM"):
            self.quizName.config(bg="red")
        ...
        # Create new file with the extension .SHIVAM
        with open(str(self.quizNameVar.get())+".SHIVAM", "w") as jsonFile:
...
def init():   
    ...
    root.minsize("700","500") # Creates static window
    root.maxsize("1000","800") # <-|
    root.title('Quiz-X by Shivam') # Seted the window name 
   
    homescreen=QuizGUI(root) # Run the object
    print("\n"*10,"\nQuiz-X Started!\t\tMade By Shivam Singh\n"+"-"*55) # Print start message
